import {getLocale, getTranslations} from "next-intl/server";
import BACCalculator from "@/components/calculator/BACCalculator";
import QuickBACCalculator from "@/components/calculator/QuickBACCalculator";
import BACTable from "@/components/content/BACTable";
import FAQ from "@/components/content/FAQ";
import FactorsGrid from "@/components/content/FactorsGrid";
import LegalLimits from "@/components/content/LegalLimits";
import MethodReferences from "@/components/content/MethodReferences";
import Footer from "@/components/layout/Footer";
import Header from "@/components/layout/Header";
import type {Locale} from "@/i18n/config";
import {Link} from "@/i18n/navigation";
import {faqSchema, webAppSchema} from "@/lib/metadata";
import {getSeoContent, type SeoPageKey} from "@/lib/seo";

function JsonLd({data}: {data: object}) {
  return (
    <script
      type="application/ld+json"
      dangerouslySetInnerHTML={{__html: JSON.stringify(data)}}
    />
  );
}

export default async function BACLandingPage({
  pageKey = "home",
}: {
  pageKey?: Extract<SeoPageKey, "home" | "calculator">;
}) {
  const locale = (await getLocale()) as Locale;
  const t = await getTranslations("landing");
  const seo = getSeoContent(locale, pageKey);

  return (
    <>
      <JsonLd data={webAppSchema} />
      {pageKey === "calculator" ? <JsonLd data={faqSchema} /> : null}
      <Header />
      <main className="min-h-screen">
        {pageKey === "home" ? (
          <>
            <section className="border-b border-border py-14 sm:py-16">
              <div className="mx-auto w-full max-w-5xl px-4 sm:px-6">
                <h1 className="max-w-4xl text-4xl leading-tight sm:text-5xl md:text-6xl">
                  {seo.h1}
                </h1>
                <p className="mt-5 max-w-3xl text-base leading-8 text-muted">
                  {t("homeSubtitle")}
                </p>
              </div>
            </section>
            <QuickBACCalculator />
            <section className="border-b border-border py-14 sm:py-16">
              <div className="mx-auto grid w-full max-w-5xl gap-5 px-4 sm:px-6 lg:grid-cols-2">
                <article className="rounded-sm border border-border bg-card p-6">
                  <h2 className="text-2xl sm:text-3xl">{t("homeHowTitle")}</h2>
                  <p className="mt-4 text-sm leading-8 text-muted">
                    {t("homeHowBody")}
                  </p>
                  <Link
                    href="/bac-calculator"
                    className="mt-5 inline-flex font-mono text-xs uppercase tracking-[0.16em] text-accent"
                  >
                    {t("homeHowCta")}
                  </Link>
                </article>

                <article className="rounded-sm border border-border bg-card p-6">
                  <h2 className="text-2xl sm:text-3xl">{t("homeLegalTitle")}</h2>
                  <p className="mt-4 text-sm leading-8 text-muted">
                    {t("homeLegalBody")}
                  </p>
                  <Link
                    href="/bac-calculator#legal"
                    className="mt-5 inline-flex font-mono text-xs uppercase tracking-[0.16em] text-accent"
                  >
                    {t("homeLegalCta")}
                  </Link>
                </article>
              </div>
            </section>
          </>
        ) : (
          <>
            <section className="border-b border-border py-20">
              <div className="mx-auto grid w-full max-w-5xl gap-12 px-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
                <div>
                  <p className="mb-4 font-mono text-xs uppercase tracking-[0.3em] text-accent">
                    {t("eyebrow")}
                  </p>
                  <h1 className="max-w-3xl text-5xl leading-tight md:text-6xl">
                    {seo.h1}
                  </h1>
                  <p className="mt-6 max-w-2xl text-base leading-8 text-muted">
                    {seo.description}
                  </p>
                  <div className="mt-6 inline-flex rounded-sm border border-danger/30 bg-danger/10 px-4 py-3 font-mono text-xs uppercase tracking-[0.16em] text-danger">
                    {t("calculatorDisclaimer")}
                  </div>
                  <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                    <a
                      href="#calculator"
                      className="inline-flex items-center justify-center rounded-sm bg-accent px-6 py-3 font-mono text-xs font-semibold uppercase tracking-[0.2em] text-black"
                    >
                      {t("primaryCta")}
                    </a>
                    <a
                      href="#learn-more"
                      className="inline-flex items-center justify-center rounded-sm border border-border px-6 py-3 font-mono text-xs font-semibold uppercase tracking-[0.2em] text-text"
                    >
                      {t("secondaryCta")}
                    </a>
                  </div>
                </div>
                <div className="rounded-sm border border-accent/20 bg-[linear-gradient(180deg,rgba(0,212,170,0.08),rgba(255,255,255,0.02))] p-6">
                  <p className="mb-4 font-mono text-xs uppercase tracking-[0.2em] text-accent">
                    {t("cardTitle")}
                  </p>
                  <div className="space-y-4 text-sm leading-relaxed text-muted">
                    {(t.raw("cardLines") as string[]).map((line) => (
                      <p key={line}>{line}</p>
                    ))}
                  </div>
                </div>
              </div>
            </section>
            <BACCalculator />
            <div id="learn-more">
              <BACTable />
              <FactorsGrid />
              <LegalLimits />
              <MethodReferences />
              <FAQ />
            </div>
          </>
        )}
      </main>
      <Footer />
    </>
  );
}
